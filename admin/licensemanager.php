...
{
	if ($add) {
		$licenseKeylist->type=$_POST["type"];
		$licenseKeylist->algo=$_POST["algo"];
		$licenseKeylist->option_code=$_POST["option_code"];
		$licenseKeylist->base_key=$_POST["base_key"];
		$licenseKeylist->name=$_POST["name"];
		$licenseKeylist->mode=$_POST["mode"];
		$licenseKeylist->multi_key_separator=$_POST["multi_key_separator"];
		$licenseKeylist->output_mode=$_POST["output_mode"];
		$licenseKeylist->duration=$_POST["duration"];
		$licenseKeylist->duration_unit=$_POST["duration_unit"];
...
} else if ($action == "external" && empty($refresh))
{
    $i=0;
    if ($remove)
    {
    ...
    {
    	$licenseList->fk_base_key = $_POST['fk_base_key'];
    	$licenseList->external_key = $_POST['external_key'];
...
if ($mode == $tabs['tab1']->mode)
{
	...
	print '<tr><td>'.$form->selectarray('fk_base_key', $licenseKeys,$_POST['fk_base_key'],0,0,0).'</td>';
