...
/**
 *   	\file       licenseorder_list.php
 *		\ingroup    licensemanager
 *		\brief      List page for licenseorder
 */
...
require_once __DIR__.'/class/licenseorder.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('licensemanager', 'licenseorder', 'write');
	$permissiontodelete = $user->hasRight('licensemanager', 'licenseorder', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("licensemanager")) {
	accessforbidden('Module licensemanager not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Licenseorder';
	$objectlabel = 'Licenseorder';
	$uploaddir = $conf->licensemanager->dir_output;
...
$title = $langs->trans("Licenseorders");
//$help_url = "EN:Module_Licenseorder|FR:Module_Licenseorder_FR|ES:Módulo_Licenseorder";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/licensemanager/licenseorder_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-licensemanager page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/licensemanager/licenseorder_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendLicenseorderRef";
$modelmail = "licenseorder";
$objecttmp = new Licenseorder($db);
