...
if (($product->id > 0) && ($action == 'update_license') && ! $_POST["cancel"] && ($user->rights->produit->creer || $user->rights->service->creer))
{
	// link license to product
	$licenseProduct->fk_product = $product->id;
	$licenseProduct->fk_base_key = $_POST["fk_base_key"];
	$licenseProduct->option_code = $_POST["option_code"];
...
if (($product->id > 0) && ($action == 'confirm_delete') && ! $_POST["cancel"] && ($user->rights->produit->creer || $user->rights->service->creer))
{
	$result=$licenseProduct->delete($user);
	$licenseProduct->id = 0;
}
	
if(($action != 'view') && ($action != 'edit_license') && ! $_POST["cancel"] && ! $_POST["delete"])
{
	if (! $result > 0) $error++;
